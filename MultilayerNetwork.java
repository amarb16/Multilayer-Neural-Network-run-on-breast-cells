...
public class MultilayerNetwork {
    ...
    public void trainEpochs(Example[] examples, int numEpochs) {
       ...
       /*
        * entire training set, it trains on it multple times
        * each example object is a different patient
        */
    }
    ...
    public static void testMultilayerNetwork() {
        ...
        Example ex = new Example(new double[] {1, 0}, new double[] {1});//inputs and the outputs
    ...
    public static void learnXorML(int numEpochs) {
        ...
        //pass the example array into the train methods
        //expect we read it from a file, make example objects make an array of that
        //have an array list of example objects
        //otherwise we'd have to change how code trains
        //nothing wrong with arraylist, but we'd just have to change a lot of things
    }
    
     public static void testBreastCancer(int numEpochs) {
       
        DataConverter data = new DataConverter();
        Example[] examples = data.getInputs();
        //System.out.println(Arrays.toString(examples));
        
        MultilayerNetwork net = new MultilayerNetwork(10, 150, 1, 1); //inputs, hidden, output, learning rate
        net.trainEpochs(examples, numEpochs);
        System.out.println(net);
        net.runTestSet(examples, 2);
    }
